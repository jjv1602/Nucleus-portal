          ...
          <Card id="card" key={single._id}>
            ...
            <Card.Body>
              ...
              {single.rsvp.includes(JSON.parse(localStorage.getItem('userInfo')).name) &&
                <Button variant="success" style={{ display: "flex" }} onClick={()=>{
                  dispatch(rsvp_remove_name(single._id));
                  
                }} 
                  
                  > <div style={{ paddingRight: "8px" }}><i className="fa-solid fa-check"></i></div>   RSVP</Button>
              }

              {
                !single.toggle &&!single.rsvp.includes(JSON.parse(localStorage.getItem('userInfo')).name) &&
                <Button onClick={()=>{
